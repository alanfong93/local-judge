...
`path` uses RFC 6901 JSON Pointer escaping. `message` is a stable contract
description, never a backend exception, stack trace, raw model output, or
prompt content.
...
contain an inability or question error. Every result is a closed object with
these fields:
...
| Score | `vote_share[level] = count(level) / n` for every level. `score = sum(level * vote_share[level])`, so it is in `[0, criteria.length - 1]` and may be fractional. | `max(level count) / n`, or null when `n = 1`. A tied mode does not invalidate a Score because its empirical expected position remains defined. |
...
It is self-contained: the engine reads the accepted request and resolved
...
request. The trace retains the exact rendered messages as well as the prompt
...
result traces keyed by question ID, and it carries the mandatory semantics
disclosure. A caller replays one trace through the normal native replay
...
profile, contract version, and evidence rationale.
